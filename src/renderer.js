...
async function boot() {
  try {
    config = await window.memix.getConfig();
    stickers = await window.memix.getStickers();
    ...
    window.memix.onStickersUpdated((list) => { 
      stickers = list; 
    });
  } catch (err) {
...
function scheduleNext() {
  ...
  loopTimer = setTimeout(advance, config.stickerIntervalMs || 7000);
}
...
async function playSound(category) {
  ...
  if (!name) return;
  try {
    const sounds = await window.memix.listSounds();
...
if (btnResize && resizeSliderWrap) {
  ...
  if (resizeSlider) {
    ...
    resizeSlider.addEventListener('input', (e) => {
      ...
      widgetSize = size;
      window.memix.resizeWindow(size, size);
      window.memix.setConfig({ widgetSize: size });
    });
  }
}
...
// ---------------- window controls ----------------
document.getElementById('btn-minimize').addEventListener('click', () => window.memix.minimizeWindow());
...
document.getElementById('btn-close').addEventListener('click', async () => {
  ...
  window.memix.closeAfterAnimation();
});
...
document.getElementById('btn-maximize').addEventListener('click', async () => {
  ...
  // Full work area — entire display usable space
  let width = 1920, height = 1080, x = 0, y = 0;
  try {
    const area = await window.memix.getWorkArea();
  ...
  window.memix.resizeWindow(width, height, 300, x, y);
  setTimeout(() => {
...
document.getElementById('dash-close').addEventListener('click', () => {
  dashboardView.animate(
    ...
    window.memix.resizeWindow(widgetSize, widgetSize, 300);
...
// ---------------- dashboard wiring ----------------
initDashboard({
  getStickers: () => window.memix.getStickers(),
  getConfig: () => window.memix.getConfig(),
  setConfig: (partial) => {
    window.memix.setConfig(partial);
    Object.assign(config, partial);
    if ('stickerIntervalMs' in partial) scheduleNext();
  },
  listSounds: () => window.memix.listSounds(),
  toggleLock: (v) => {
    window.memix.toggleLock(v);
