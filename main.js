...
function ensureDir(dir) {
  ...
  catch (err) { console.error('[pocket] mkdir failed for', dir, err); }
}
...
function writeConfigNow() {
  ...
  catch (err) { console.error('[pocket] save config failed:', err); }
}
...
function scanStickers() {
  ...
  try {
    ...
    console.error('[pocket] scan failed:', err);
    return [];
  }
  return entries
...
function startStickerWatcher() {
  ...
  try {
    ...
    stickerWatcher.on('error', (err) => {
      console.error('[pocket] watcher error:', err);
      stopStickerWatcher();
    });
  } catch (err) {
    console.error('[pocket] could not start watcher:', err);
  }
}
...
ipcMain.handle('screen-work-area', () => screen.getPrimaryDisplay().workArea);
...
ipcMain.handle('list-sounds', () => {
  ...
  try {
    ...
    console.error('[pocket] list-sounds failed:', err);
    return [];
  }
});
...
ipcMain.on('resize-window', (_e, { width, height, animateMs = 0 } = {}) => {
  if (!win || win.isDestroyed()) return;
  const w = Math.max(120, Math.round(Number(width)  || 0));
  const h = Math.max(120, Math.round(Number(height) || 0));
  if (!w || !h) return;
  win.setResizable(true);
  win.setSize(w, h, !!animateMs);   // animate flag is macOS-only, harmless elsewhere
});
